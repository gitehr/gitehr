...
- **Keep `gitehr init` as a standalone-repo bootstrap**: rejected. With no existing users there is no back-compat cost, and a second entry point undercuts the single model. `gitehr store init` is the one front door.
