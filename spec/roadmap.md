...
- [x] **R71 - Evaluate Open Wearables for streamed vitals integration:** assess [the-momentum/open-wearables](https://github.com/the-momentum/open-wearables) as a self-hostable unified API for wearable device data (Garmin, Whoop, Apple HealthKit, etc.) that could feed observations into GitEHR through an import adapter. MIT-licensed, FastAPI + React, early-stage. Evaluated in [`docs/design/open-wearables-evaluation.md`](../docs/design/open-wearables-evaluation.md): not adopted yet, pending R25/R26 and the streaming-vitals design.
