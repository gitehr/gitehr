...
- [x] **R31 - Add MCP prompt templates:** SOAP note, discharge summary, referral, consultation, and medication review. Implemented in `cli/src/commands/mcp/server_impl/prompts.rs` - stateless `PromptHandler` renders each template's drafting instructions from its arguments (no repository access), wired into `prompts/list`/`prompts/get` in `server.rs`. Documented in `docs/cli/mcp-usage.md`.
