...
- [~] **R30 - Add MCP tool handlers:** journal/state mutations, search, repository-policy checks, and clinical calculation through clincalc. `add_journal_entry` writes a real entry - as an **uncommitted draft pending human approval** per [ADR-0007](adr/0007-mcp-writes-are-drafts-until-approved.md) (`journal drafts --approve/--reject`); `update_state` and `search_repository` were already real. Repository-policy checks and clincalc integration remain outstanding.
- [x] **R31 - Add MCP prompt templates:** SOAP note, discharge summary, referral, consultation, and medication review. Implemented in `cli/src/commands/mcp/server_impl/prompts.rs` - stateless `PromptHandler` validates bounded string arguments and renders source-grounded drafting instructions without repository access, wired into `prompts/list`/`prompts/get` in `server.rs`. Documented in `docs/cli/mcp-usage.md`.
- [ ] **R32 - Add MCP authentication:** start with `.gitehr/mcp-tokens.json`, then define OAuth2 and mTLS options for remote deployments.
- [x] **R33 - Add MCP audit logging to journal entries.** Every successful `tools/call` now writes a dedicated audit journal entry, distinct from any entry the tool itself wrote, with the `mcp_audit: {method, tool, result}` front matter shape from this spec (`cli/src/commands/mcp/server_impl/audit.rs`, wired into `ToolHandler::call_tool` in `tools.rs`). Best-effort and success-only: a failure to write the audit trail is logged to stderr rather than failing the tool call, and nothing is written for a call that errors before touching the repository. Client/session identity (client name/version, token, IP) is omitted rather than faked, since GitEHR does not track it yet (depends on R32/R35).
...
- [x] **R77 - Finish the interrupted security review:** completed 2026-08-31. Audited and cleared: `import.rs` (destination filenames come from `Path::file_name()`, which can't contain `/` or `..`; import walks use `file_type().is_file()`, so symlinks in an import source are skipped, not followed), `store/*.rs` (the only user-supplied string reaching a path join, the subject name, is `slugify()`d to alphanumerics/hyphens first), `mcp/server_impl/server.rs` (thin JSON-RPC dispatcher, no path joins or command execution of its own). Confirmed: the locked `tar` crate is 0.4.46 (`Cargo.lock`), above the RUSTSEC-2021-0080 fix line. Found and fixed two symlink-following issues, a variant of the R73/R74 trusted-base-directory pattern expressed via symlinks rather than `..` segments: `transport/create.rs` used `Path::is_file()` (follows symlinks) instead of `WalkDir`'s cached `file_type()`, so a symlink committed under `journal/`, `state/`, `imaging/`, `documents/`, or `.gitehr/` in a received or cloned repository would have its target's contents read and embedded in the transport archive - fixed by switching to `entry.file_type().is_file()`. `upgrade.rs` and `upgrade_binary.rs` read and wrote `.gitehr/GITEHR_VERSION` and `.gitehr/gitehr` without checking for symlinks first, so the same untrusted-repo scenario could disclose an arbitrary locally-readable file's contents (printed as the "current version") or overwrite an attacker-chosen path with the running `gitehr` binary and mark it executable - fixed by rejecting any of those paths that resolve as a symlink (`reject_symlink`, shared by both files) before reading, writing, or `chmod`ing them. Regression tests added in `cli/tests/commands/transport.rs` and `cli/tests/commands/upgrade.rs`.

- [x] **R79 - Stop `gitehr encrypt` claiming encryption that never happens:** `cli/src/commands/encrypt.rs` writes `.gitehr/ENCRYPTED` and prints "Repository marked as encrypted" while encrypting nothing - every clinical file stays plaintext - and `decrypt.rs` merely deletes the marker. This is dangerous false assurance in a clinical tool. Remediation: until R67/R68 land, make both commands exit non-zero with a clear "not yet implemented" error and write no marker; alternatively hide them behind a feature flag. Verified 2026-08-29.
- [x] **R80 - Terminate git option parsing in `git_add`:** `git_add` (`cli/src/commands/git.rs:31-33`) passes the path positionally, so a filename beginning with `-` would be parsed by git as a flag. Use `git add -- <path>` (and audit other callers of `run_git_command` for the same). Low severity (callers currently pass generated filenames); verified 2026-08-29. `git.rs` is otherwise sound - args go through `Command::args` with no shell involved.
- [x] **R81 - `gitehr transport create --encrypt` produces an unencrypted archive but reports success:** `cli/src/commands/transport/create.rs` honours `--encrypt` only by printing "Transport encryption is not yet implemented", then returns `Ok(())` having written a plaintext `.tar.gz`. Same false-assurance class as R79 - a user who asked for encryption gets none and is not told it failed. Remediation: make `--encrypt` exit non-zero ("not yet implemented") until transport encryption exists (R68), rather than downgrading silently. Verified 2026-08-29. Note: `create.rs` bundles the entire `.gitehr/` directory (including any `.gitehr/gitehr` / `.gitehr/gitehr-gui`) into the archive, which is the delivery vector for R78; it does force `0o644` on every entry, so a gitehr-created archive strips the executable bit, but a hand-crafted archive or clone does not - R78 stands.
...
- [x] **R76 - Validate the repository before serving MCP:** `gitehr mcp serve` now refuses to start unless `--repo-path` (or the current directory) contains a `.gitehr` directory, and refuses an encrypted repository (`.gitehr/ENCRYPTED` present) rather than serving it. See `cli/src/commands/mcp/serve.rs::validate_repo`.
...
- [x] **R78 - Untrusted bundled GUI binary is executed from the repo directory:** fixed 2026-09-04. `gitehr gui` now prefers an installed `gitehr-gui` on `$PATH` and only launches a bundled `.gitehr/gitehr-gui` (or `.gitehr/gitehr-gui.exe`) when the caller passes `--allow-bundled`; without it, a bundled binary is reported in the error rather than executed (`cli/src/commands/gui.rs::run`), closing the arbitrary-code-execution path where a received repository (clone or transport archive) carries a hostile executable at that path. Chose the explicit-opt-in-flag option over a persisted per-repo trust marker, since a marker committed inside the repository could be supplied by the same untrusted party it's meant to gate. Audited the other repo-relative-execution candidates named in the original finding: `plugin.rs` only ever searches `$PATH` directories (`discover`/`discover_in`), never a repo-relative one, so it carries no equivalent risk; `.gitehr/gitehr` (the bundled CLI copy) is only ever written to by `upgrade.rs`/`upgrade_binary.rs`, never executed by any gitehr command. Tests in `cli/tests/commands/gui.rs` cover the refusal, the opt-in launch, and that a `$PATH` install is preferred over an unallowed bundled binary.
...
- [x] **R9 - Add the FHIR layout to the template:** `/fhir/` added to the folder template (`definitions/`, `resources/`, `indexes/`) with a README documenting the layout and immutable-version conventions from `spec/fhir.md`; the layout ships in every new repo via the template, and transport archives include it (a unit test in `transport/create.rs` now fails if any template directory is left out of an archive). No FHIR commands yet - those are R12-R14; the definitions source (pinned official release vs compiled FSH) remains an R11 decision, and the layout is expected to hold both R5 and R4 instances, so each records its own release.
