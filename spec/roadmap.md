...
- [ ] **R77 - Finish the interrupted security review:** the following were never successfully read and are unaudited: `upgrade_binary.rs`, `upgrade.rs`, `encrypt.rs`/`decrypt.rs`, `import.rs`, `git.rs`, `store/*.rs`, `mcp/server_impl/server.rs`, and `transport/create.rs`. Priorities: any user-supplied path or filename joined onto a base directory (the R73/R74 traversal pattern), anything downloading or executing binaries (`upgrade*`), shell-out argument construction (`git.rs`), and key handling (`encrypt`/`decrypt`). Also verify the locked `tar` crate is >= 0.4.36 (RUSTSEC-2021-0080); this was claimed as 0.4.46 during review but not reliably confirmed.
