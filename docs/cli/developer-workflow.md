...
The repo has one released Rust crate and one product release. MCP is built into the `gitehr` binary as `gitehr mcp serve`, so `release-plz.toml` treats `gitehr` as the only release package and keeps plain `vX.Y.Z` tags rather than package-prefixed tags.
